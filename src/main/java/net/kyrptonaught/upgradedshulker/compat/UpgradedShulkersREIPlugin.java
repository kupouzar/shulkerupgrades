...
public class UpgradedShulkersREIPlugin implements REIClientPlugin {
    ...
    private void registerColorRecipe(DisplayRegistry recipeHelper, ShulkerUpgrades.MATERIAL material, DyeColor color, ItemStack output) {
        ...
        defaultedList.add(Ingredient.fromTag(TagKey.of(RegistryKeys.ITEM, new Identifier("us", material.name().toLowerCase() + "_shulkers"))));
        defaultedList.add(Ingredient.ofItems(DyeItem.byColor(color)));
        ShapelessRecipe shapelessRecipe = new ShapelessRecipe(new Identifier(UpgradedShulkerMod.MOD_ID, "dyeshulkers"), "dyeshulkers", CraftingRecipeCategory.MISC, output, defaultedList);
    ...
    private void registerTierUpRecipe(DisplayRegistry recipeHelper, ShulkerUpgrades.MATERIAL material, DyeColor color, ItemStack output) {
        ...
        ShapedRecipe recipe = new ShapedRecipe(new Identifier(UpgradedShulkerMod.MOD_ID, "upgradeshulkertier"), "upgradeshulkertier", CraftingRecipeCategory.MISC, 3, 3, defaultedList2, output);
